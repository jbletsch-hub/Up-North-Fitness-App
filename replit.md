...
- **Facial Hair Redesign (Nov 12, 2025):** Completely rebuilt facial hair rendering to match the natural hair style using smooth bezier curves instead of geometric shapes. Small beard uses buzzcut-style rendering (close to face with subtle texture lines). Big beard uses short hair-style rendering (flowing curved paths with more volume). Both beards positioned at bottom of face (cy=70-88) with clear mouth opening (mouth at cy=68). Stubble remains as light dots pattern.
- **MVL Race Scope Change (Nov 13, 2025):** Modified MVL (Most Valuable Lifter) daily competition to only count XP from daily activities (check-ins, weigh-ins, PRs, photos, calories, challenges). Goal completion XP (weekly 100 XP, yearly 2000 XP, lifetime 5000 XP) no longer counts toward MVL race standings. This ensures daily leaderboard reflects consistent daily effort rather than occasional large goal completions.
- **iOS Input Fix (Nov 13, 2025):** Fixed critical iOS Safari bug preventing users from typing into calorie and goal input fields. Changed from `type="number"` to `type="text"` with `inputMode="numeric"` to trigger numeric keyboard while allowing text input on all mobile devices.
