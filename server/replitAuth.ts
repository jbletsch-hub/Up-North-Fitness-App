...
async function upsertUser(claims: any) {
  ...
  let username = email?.split("@")[0] || firstName || `user${userId}`;
...
export async function setupAuth(app: Express) {
  ...
  // Admin password login - for admin/admin123
  app.post("/api/admin-login", async (req, res) => {
    ...
    if (username === "admin" && password === "admin123") {
      ...
      // Create session
      (req as any).login({ claims: { sub: adminUser.id }, isAdmin: true }, (err: any) => {
        if (err) {
...
export const isAuthenticated: RequestHandler = async (req, res, next) => {
  ...
  if (!req.isAuthenticated() || (!user.expires_at && !user.isAdmin)) {
    return res.status(401).json({ message: "Unauthorized" });
  }

  // Admin login doesn't have expires_at
  if (user.isAdmin) {
    return next();
  }
